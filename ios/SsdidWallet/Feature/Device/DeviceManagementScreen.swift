...
struct DeviceManagementScreen: View {
                ...
                ScrollView {
                    LazyVStack(spacing: 10) {
                        ...
                        if otherDevices.isEmpty {
                            ...
                            ForEach(otherDevices, id: \.keyId) { device in
                                VStack(alignment: .leading, spacing: 0) {
                                    HStack {
        ...
        .alert("Revoke Device", isPresented: Binding(
            ...
            Button("Revoke", role: .destructive) {
                if let key = pendingRevokeKey {
                    Task { await revokeDevice(targetKeyId: key) }
    ...
    private func loadDevices() async {
        ...
        guard let id = await services.vault.getIdentity(keyId: keyId) else { return }
        identity = id

        let deviceManager = makeDeviceManager()
        do {
            devices = try await deviceManager.listDevices(identity: id)
        } catch {
            revokeError = "Failed to load devices: \(error.localizedDescription)"
        }
    }

    private func revokeDevice(targetKeyId: String) async {
        ...
        let deviceManager = makeDeviceManager()
        do {
            try await deviceManager.revokeDevice(identity: id, targetKeyId: targetKeyId)
            // Reload device list after successful revocation
            devices = try await deviceManager.listDevices(identity: id)
        } catch {
